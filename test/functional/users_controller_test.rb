...
class UsersControllerTest < ActionController::TestCase
  ...
  test "should ban user if admin" do
    login_as :Administrator
    ban_time = 1.year.from_now
    put :update, :id => users(:trevor).id, :user => {:ban_message => 'get banned!', :banned_until => ban_time}
    users(:trevor).reload
    assert_equal users(:trevor).ban_message, 'get banned!'
    assert users(:trevor).banned?
  end
