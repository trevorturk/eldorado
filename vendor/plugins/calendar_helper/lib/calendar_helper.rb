...
module CalendarHelper
  ...
  def calendar(options = {}, &block)
    ...
    cal << %(<thead><tr class="#{options[:month_name_class]}"><th colspan="7"><span style="float:left;">#{Date::MONTHNAMES[options[:month]]} #{options[:year]}</span><span style="float:right;">#{render :partial => 'nav'}</span></th></tr><tr class="#{options[:day_name_class]}">)
