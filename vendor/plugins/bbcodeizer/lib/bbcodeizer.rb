module BBCodeizer  
  class << self
    # generates a random id for AJAXified tags
    def random_id
    ...
    Tags = {
      ...
      :start_spoiler         => [ /\[spoiler\]/i, proc { id = BBCodeizer::random_id(); %Q{ <a href="#" class="spoiler-link" onclick="$(\'#{id}\').toggle(); return false;">SPOILER</a><div id="#{id}" class="spoiler" style="display:none;"> } } ],
      :end_spoiler           => [ /\[\/spoiler\]/i, '</div>' ],
      :start_nsfw            => [ /\[nsfw\]/i, proc { id = BBCodeizer::random_id(); %Q{ <a href="#" class="nsfw-link" onclick="$(\'#{id}\').toggle(); return false;">NSFW</a><div id="#{id}" class="nsfw" style="display:none;"> } } ],
      :end_nsfw              => [ /\[\/nsfw\]/i, '</div>' ],
  ...
  private
    ...
    def on_matching_tags(tag_name, string)
      if string.scan(Tags[:"start_#{tag_name}"].first).size == string.scan(Tags[:"end_#{tag_name}"].first).size
        yield
      end
    end

    def code(string)
      on_matching_tags(:code, string) do
        apply_tags(string, :start_code, :end_code)
      end
    end
  
    def quote(string)
      on_matching_tags(:quote, string) do
        apply_tags(string, :start_quote_with_cite, :start_quote_sans_cite, :end_quote)
      end
    end

    def spoiler(string)
      on_matching_tags(:spoiler, string) do
        apply_tags(string, :start_spoiler, :end_spoiler)
      end
    end

    def nsfw(string)
      on_matching_tags(:nsfw, string) do
        apply_tags(string, :start_nsfw, :end_nsfw)
      end
    end

    def apply_tags(string, *tags)
      tags.each do |tag|
        regex, substitution = Tags[tag]
